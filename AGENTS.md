...
2. **Open a PR** for the change (never push to `main` directly — PR-only). Use `gh pr create` with a Conventional Commit message and a body that references the issue (`Resolves #NN`). **AI-created PRs MUST be labeled `generated`** (e.g. `gh pr edit <n> --add-label generated`) — the `human-approval-gate` required check blocks unapproved `generated` PRs, and human PRs pass without it.
