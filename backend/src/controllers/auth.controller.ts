...
export default async function restrictMediawikiAccess(
  ...
  try {
    ...
    const allowedPrefixRegEx = new RegExp(
      `^/wiki/(${wikiadviserLanguagesRegex})/(images/thumb|load.php\\?|api.php\\?action=query\\&format=json\\&(meta=(filerepoinfo|siteinfo)|prop=imageinfo\\&indexpageids=1&iiprop=size%7Cmediatype\\&titles=)|(skins|resources|extensions/UniversalLanguageSelector|images/timeline)/|extensions/Kartographer)`,
      'i'
    );
