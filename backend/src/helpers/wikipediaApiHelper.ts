...
export async function getArticleWikiText(title: string) {
  // Fetch the wikitext of the Wikipedia Article.
  // The article in Wikipedia
  const wpArticleUrl = `https://fr.wikipedia.org/w/api.php?action=query&formatversion=2&prop=revisions&rvprop=content&rvslots=%2A&titles=${title}&format=json`;
