...
for env in "${environments[@]}"; do
    ...
    for lang in "${languages[@]}"; do
        ...
        then
             ...
             MEDIAWIKI_SECRET_KEY=""$env"_wiki_"$lang"" MEDIAWIKI_UPGRADE_KEY=""$env"_wiki_"$lang""   envsubst '$SERVER_ENDPOINT $URL_PATH $LANGUAGE $DB_NAME $DB_USER $DB_PASSWORD $MEDIAWIKI_SECRET_KEY $MEDIAWIKI_UPGRADE_KEY' < LocalSettings.php > ./LocalSettings_prod_"$lang".php
        else
...
# Gadgets
wget https://extdist.wmflabs.org/dist/extensions/Gadgets-REL$extension_version-$Gadgets_version.tar.gz
for environment in "${environments[@]}"; do
    for lang in "${languages[@]}"; do
...
# TimedMediaHandler
wget https://extdist.wmflabs.org/dist/extensions/TimedMediaHandler-REL$extension_version-$TimedMediaHandler_version.tar.gz
for environment in "${environments[@]}"; do
    for lang in "${languages[@]}"; do
        tar -xzf TimedMediaHandler-REL$extension_version-$TimedMediaHandler_version.tar.gz -C /var/www/wiki-$environment/$lang/extensions/
    done
done
