...
export async function refineArticleChanges(
  ...
  $CheerioAPI("[data-diff-action]:not([data-diff-action='none'])").each(
    (_index, element) => {
      ...
      const isComment = Boolean($element.find(".ve-ce-commentNode").length);
      if (isComment) {
...
export async function processExportedArticle(
  ...
  let processedData = pageContentXML;
  if (ENV.WIKIPEDIA_PROXY) {
  ...
  console.log("exported length", pageContentXML.length);
