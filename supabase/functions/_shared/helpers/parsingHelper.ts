import { Cheerio, CheerioAPI, Element, load } from "npm:cherio@^1.0.0-rc.2";
    ...
    (_index: number, element: Element) => {
      ...
      if (diffAction === "structural-change") {
        ...
        descriptionListItems.each((_i: number, elem: Element) => {
          ...
          $CheerioAPI(elem)
            .find("del")
            .replaceWith(function strikeThrough(this: Element) { //I change this to Element type
              return `${createStrikethroughText($CheerioAPI(this).text())} `; // E.g. <div><del>2017</del><ins>1999</ins></div> returns '2̶0̶1̶7̶ 1999'
            });

          $CheerioAPI(elem)
            .find("li")
            .each((_ulElemIndex: number, ulElem: Element) => {
      ...
      $wrapElement.find("[data-diff-id]").each((_: number, el: Element) => {
        $CheerioAPI(el).removeAttr("data-diff-id");
      });
      $wrapElement.find("[data-parsoid]").each((_: number, el: Element) => {
